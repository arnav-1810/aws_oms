workflow omsinput_cloudtrail {
   Param($oms_input)
  	inlineScript { 
      $loganalytics_json = $Using:oms_input   
...
#				Submit the data to the API endpoint
			Post-OMSData -customerId $customerId -sharedKey $sharedKey -body ([System.Text.Encoding]::UTF8.GetBytes($loganalytics_json)) -logType $logType
	}
}
